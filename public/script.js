...
function fetchFirstOk(urls){
  function tryOne(idx, resolve, reject){
    ...
    fetchNoCache(url).then(function(r){
      ...
      r.json().then(function(json){ resolve({ url: url, json: json }); })
...
function loadCart(){
  try{
    var raw = localStorage.getItem(CART_KEY); if(!raw) return;
    var arr = JSON.parse(raw); cart.clear();
...
function loadCheckout(){
  try{
    var raw = localStorage.getItem(CHECKOUT_KEY); if(!raw) return;
...
function computeBestPriceRaw(product, qtyRaw){
  ...
  if (!hasTiers){
    ...
    return { qty, requestedQty: qty, total: total0, breakdown:[{ qty: qty, times:1, price:+cfg.basePrice.toFixed(2)}], pricingMode:'base', adjusted:false };
  }
  ...
  if (!tiers.length){
    ...
    return { qty, requestedQty: qty, total: total1, breakdown:[{ qty: qty, times:1, price:+cfg.basePrice.toFixed(2)}], pricingMode:'base', adjusted:false };
  }
  var INF = 1e15;
  ...
  if (dp[N] === INF){
    ...
    return { qty, requestedQty: qty, total: total2, breakdown:[{ qty: qty, times:1, price:+cfg.basePrice.toFixed(2)}], pricingMode:'base', adjusted:false };
  }
  var used = new Map();
  var k = N;
  while (k > 0){
  ...
  var total = 0;
  var breakdown = [];
  used.forEach(function(times, jj){
    var tt = tiers[jj];
    total += times * tt.price;
    breakdown.push({ qty: tt.rawQty, times: times, price: tt.price });
  });
  total = +total.toFixed(2);
  breakdown.sort(function(a,b){ return a.qty - b.qty; });
  return { qty, requestedQty: qty, total: total, breakdown: breakdown, pricingMode:'combos', adjusted:false };
}
function bestPriceById(id, qty){
  ...
  var hasEmbeddedTiers = (p && p.bundlePricing && Array.isArray(p.bundlePricing.tiers) && p.bundlePricing.tiers.length)
...
function openModal(modal){
  ...
  setTimeout(function(){
    var x = modal.querySelector('.modal__close, [data-close]');
...
function updateFavCardQtyUI(id){
  ...
  if (item){
    ...
    else { if (addBtn) addBtn.remove(); meta.insertAdjacentHTML('beforeend', html); }
  } else {
    if (!addBtn){
...
function renderFavoritesModal(){
  ...
  var items = Array.from(favorites).map(function(id){ return productsById.get(id); }).filter(Boolean);
  favsModalList.innerHTML = items.map(function(p){
    ...
    var actions = isAvailable ? (it ? ('<div class="actions">'+ favQtyMarkup(p.id, it.qty, it.step || (p.soldBy==='weight'?0.25:1)) +'</div>') : ('<div class="actions"><button class="btn ghost favs-add">Agregar</button></div>')) : ('<div class="actions"><button class="btn add unavailable" type="button" disabled aria-disabled="true">No disponible</button></div>');
    return '' +
  ...
  favsModalList.onclick = function(e){
    ...
    if (e.target.closest('.favs-add')){
      ...
      var unitLabel = p.soldBy === 'weight' ? 'kg' : 'pza';
      cart.set(id, { id:id, name:p.name, unit:+Number(p.price||0), qty:minQty,
                     soldBy:p.soldBy||'unit', unitLabel:unitLabel,
                     step:Number(p.step|| (p.soldBy==='weight'?0.25:1)), minQty:minQty });
      renderCart(); syncCardsQty(id); renderFavoritesModal(); updateFavCardQtyUI(id);
      return;
    }

    if (e.target.closest('.favs-remove')){
      toggleFav(id); renderFavoritesModal(); renderFavoritesRail(); updateAllCardHearts(); return;
    }
    ...
    if (qplus || qminus){
      ...
      if (qplus){
        ...
        if (next < (minQ - EPS)) { if (confirm('¿Eliminar este artículo del carrito?')) { cart.delete(id); } else { return; } }
        else { if (it.soldBy !== 'weight') next = Math.max(1, Math.round(next)); it.qty = next; cart.set(id, it); }
      }
      renderCart(); syncCardsQty(id); renderFavoritesModal(); updateFavCardQtyUI(id);
      return;
    }
...
function renderProductGrid(products){
  ...
  var html = products.map(function(p){
    ...
    var allTiers = (p && p.bundlePricing && Array.isArray(p.bundlePricing.tiers) && p.bundlePricing.tiers)
    ...
    '<article class="card' + (isAvailable ? '' : ' is-unavailable') + '"' +
      ...
      '<div class="media">' +
        ...
        '<button class="fav-heart' + (isFav(id) ? ' is-fav' : '') + '"' +
          ...
          ' aria-label="Marcar favorito">' + favHeartSVG(isFav(id)) + '</button>' +
      '</div>' +
      '<div class="info">' +
        '<h3>' + name + '</h3>' +
...
function buildCategoryFilters(products){
  ...
  if (categorySelect){
    ...
    categorySelect.innerHTML = '<option value="">Todas</option>' + Array.from(cats).sort().map(function(c){ return '<option>' + c + '</option>'; }).join('');
  ...
  if (subcategorySelect){
    ...
    subcategorySelect.innerHTML = '<option value="">Todas</option>' + Array.from(subs).sort().map(function(s){ return '<option>' + s + '</option>'; }).join('');
...
function renderCart(){
  ...
  items.forEach(function(it){
    ...
    li.className = 'cart-item'; li.dataset.id = it.id;
    li.innerHTML = '' +
      '<div class="row">' +
  ...
  cartBadge.textContent = totalQty; cartBadge.hidden = totalQty === 0;
  ...
  if (needsScroll) {
    ...
    cartList.style.maxHeight = ''; cartList.style.overflowY = '';
  }
  saveCart();
  updateCheckoutTotalPill();
  fsShowOnAdd = true; updateFreeShippingPromo();
  refreshAllFavQtyUIs();
}
...
cartList.addEventListener('click', function(e){
  ...
  var id = li.dataset.id; var item = cart.get(id); if (!item) return;
  ...
  if (plus) {
    ...
    renderCart(); syncCardsQty(id); updateFavCardQtyUI(id); return;
  }
  if (minus) {
    ...
    if (next < (minQty - EPS)) {
      ...
      var q2 = next;
      if (item.soldBy !== 'weight') q2 = clampUnits(q2);
      item.qty = q2; cart.set(id, item);
      renderCart(); syncCardsQty(id); updateFavCardQtyUI(id);
    }
  }
});
...
function createQtyControl(card, qty){
  ...
  btnPlus.addEventListener('click', function(){
    ...
    fsShowOnAdd = true; renderCart(); updateFavCardQtyUI(id);
    if (modalFavs && modalFavs.classList.contains('open')) renderFavoritesModal();
  });
  btnMinus.addEventListener('click', function(){
    ...
    if (next < (minQty - EPS)) {
      ...
      cart.set(id, cur); spanQty.textContent = fmt(cur.qty);
      renderCart(); updateFavCardQtyUI(id); if (modalFavs && modalFavs.classList.contains('open')) renderFavoritesModal();
    }
  });
  return control;
}
function switchToQtyControl(card, startQty, addToCart){
  if (startQty===void 0) startQty = 1; if (addToCart===void 0) addToCart = false;
  var info = getCardInfo(card);
  ...
  if (addToCart) { cart.set(info.id, Object.assign({}, info, { qty: start })); renderCart(); }
  var addBtn = card.querySelector('.btn.add');
...
function syncCardsQty(id){
  Array.prototype.forEach.call(document.querySelectorAll('.card'), function(card){
    ...
    if (item) {
      if (!qtyControl) { switchToQtyControl(card, item.qty, false); }
      else { var span = qtyControl.querySelector('span'); if (span) span.textContent = (item.soldBy === 'weight') ? formatQty(item.qty, item.step) : formatQty(item.qty, 1); }
    } else { if (qtyControl) qtyControl.replaceWith(createAddButton(card)); }
  });
}
...
function applyFilters(){
  ...
  var cards = getCards(); var visibleCount = 0;
  if (searchVal) {
    hideEmpty();
    ...
    if (visibleCount === 0){ showEmpty('Verifique que escribió bien el nombre del producto | Posiblemente no haya inventario de este producto.'); }
    else { hideEmpty(); }
    updateAllCardHearts(); return;
  }
  var catVal = categorySelect.value; var hasCategory = !!catVal;
  subcategorySelect.disabled = !hasCategory; if (!hasCategory) subcategorySelect.value = '';
  cards.forEach(function(card){
  ...
  if (!hasCategory) { if (visibleCount === 0) showEmpty('Por el momento no hay productos para mostrar.'); else hideEmpty(); }
  else {
    if (visibleCount === 0) { if (subcategorySelect.value) showEmpty('Aún no contamos con artículos en esta subcategoría.'); else showEmpty('Aún no contamos con artículos en esta categoría.'); }
    else hideEmpty();
  }
  updateAllCardHearts();
}
...
function loadProducts() {
  return Promise.allSettled(API.productos.map(function(u){
    return fetchNoCache(u).then(function(r){ if (!r.ok) throw new Error('!ok'); return r.json(); });
  }))
  .then(function(results){
    ...
    arrays.forEach(function(list){
      list.forEach(function(raw){
        var id = (raw.id != null ? raw.id : raw.name); id = id != null ? String(id) : ''; if (!id) return;
        var p = Object.assign({}, raw); p.id = id; p.active = toBoolActive(p.active); byId.set(id, p);
      });
    });
    var all = Array.from(byId.values());
    var visible = all.map(function(p){ return Object.assign({}, p, { active: (p.active !== false) }); });
    productsById.clear(); visible.forEach(function(p){ productsById.set(p.id, p); });
    renderProductGrid(visible); buildCategoryFilters(visible); bindAddButtons(); bindFavoriteHearts(); applyFilters(); renderFavoritesRail();
  }).catch(function(e){
    ...
    renderProductGrid(demo); buildCategoryFilters(demo); bindAddButtons(); bindFavoriteHearts(); applyFilters();
  });
}
function loadZones() {
  return fetchFirstOk(API.zonas).then(function(res){
    var json = res.json; var zonas = [];
    ...
    zonesByName.clear(); ZONES.forEach(function(z){ zonesByName.set(z.nombre, { cost: z.costo, threshold: z.freeShippingThreshold }); });
    zone.innerHTML = ['<option value="">Selecciona una zona…</option>'].concat(
      ZONES.map(function(z){ return '<option value="' + z.nombre + '|' + z.costo.toFixed(2) + '" data-th="' + (z.freeShippingThreshold || 0) + '">' + z.nombre + ' — $' + z.costo.toFixed(2) + '</option>'; })
    ).join('');
  }).catch(function(e){
...
Array.prototype.forEach.call(document.querySelectorAll('[data-close="cart"]'), function(el){
  el.addEventListener('click', function(){ closeModal(modalCart); });
});
Array.prototype.forEach.call(document.querySelectorAll('[data-close="checkout"]'), function(el){
  el.addEventListener('click', function(){ closeModal(modalCheckout); });
});
Array.prototype.forEach.call(document.querySelectorAll('[data-close="confirm"]'), function(el){
  el.addEventListener('click', function(){ closeModal(modalConfirm); });
});
btnCancel.addEventListener('click', function(){ closeModal(modalCheckout); });
...
(function attachTransferNotice(){
  ...
  Array.prototype.forEach.call(radios, function(r){
    r.addEventListener('change', function(e){
      ...
      if (v === 'Transferencia') {
        if (!transferNoticeShown) { if (modalTransfer) openModal(modalTransfer); transferNoticeShown = true; }
...
function getSelectedZoneInfo(){
  ...
  var parts = val.split('|');
  var name = parts[0]; var costStr = parts[1];
...
function updateFreeShippingPromo(){
  ...
  if (!name || !(threshold > 0)) { fsPill.classList.remove('show'); fsPill.classList.add('hidden'); return; }
...
function showFsToastOnce() {
  ...
  fsLastText = current; clearTimeout(fsTimer);
  fsPill.classList.remove('hidden'); void fsPill.offsetWidth; fsPill.classList.add('show');
  fsTimer = setTimeout(function(){ fsPill.classList.remove('show'); setTimeout(function(){ fsPill.classList.add('hidden'); }, FS_COOLDOWN_MS); }, FS_SHOW_MS);
}
zone.addEventListener('change', function(){ validateCheckout(); updateCheckoutTotalPill(); fsShowOnAdd = true; updateFreeShippingPromo(); });
...
var __adsTimers = []; var __adsRunning = false; var __adsIndex = 0;
/* Util: fecha/hora */
function nowMs(){ return Date.now(); }
function snoozedUntil(){ return parseInt(localStorage.getItem(ADS_SNOOZE_KEY)||'0',10) || 0; }
function setSnooze(minutes){ var until = nowMs() + Math.max(1, minutes) * 60 * 1000; localStorage.setItem(ADS_SNOOZE_KEY, String(until)); }
/* Crear DOM si hace falta */
function ensureAdsDOM(){
  if (__adsNode) return;
  var sec = document.createElement('section');
  sec.id = 'popupKachu';
  sec.setAttribute('role','dialog'); sec.setAttribute('aria-live','polite'); sec.setAttribute('aria-label','Notificación');
  sec.innerHTML = '' +
  ...
  __adsL1 = sec.querySelector('.l1'); __adsL2 = sec.querySelector('.l2'); __adsCta = sec.querySelector('.lcta .cta');
  __adsClose.addEventListener('click', function(){ stopAdsCycle(true); setSnooze(ADS_SNOOZE_MIN); });
}
/* Render del mensaje */
function setAdsContent(msg){
  ...
  if (hasCTA){ __adsCta.textContent = msg.ctaText; __adsCta.href = msg.ctaUrl; pcta.style.display = ''; }
  else { __adsCta.removeAttribute('href'); pcta.style.display = 'none'; }
}
/* Animaciones */
function adsAnimateIn(){ __adsNode.classList.remove('out'); __adsNode.style.display = 'block'; void __adsNode.offsetWidth; __adsNode.classList.add('in'); }
function adsAnimateOut(cb){
  __adsNode.classList.remove('in'); __adsNode.classList.add('out');
  var t = setTimeout(function(){ __adsNode.style.display = 'none'; cb && cb(); }, 240);
  __adsTimers.push(t);
}
/* Ciclo */
...
function stopAdsCycle(hideNow){
  ...
  __adsTimers = [];
  if (__adsNode && hideNow){ __adsNode.classList.remove('in','out'); __adsNode.style.display = 'none'; }
}
...
checkoutForm.addEventListener('submit', function(e){
  ...
  if (splitInfo && splitInfo.splitEnabled){
    if (splitInfo.paid + 1e-6 < splitInfo.total){ alert('Faltan ' + formatMoney(splitInfo.total - splitInfo.paid) + ' para cubrir el total.'); return; }
    totalDue = splitInfo.total;
    ticket = buildTicket({ items: items, zoneName: zoneInfo.name, shipping: shipping, pay: 'Pago dividido', subtotal: subtotal, totalDue: totalDue, address: address.value.trim(), split: splitInfo });
  } else {
    ...
    ticket = buildTicket({ items: items, zoneName: zoneInfo.name, shipping: shipping, pay: pay, subtotal: subtotal, totalDue: totalDue, address: address.value.trim(), efectivo: efectivo });
  }
...
function getCartItemsDetailed(){
  ...
  cart.forEach(function(it){
    items.push({ id: it.id, name: it.name, unit: Number(it.unit), qty: Number(it.qty), line: bestPriceById(it.id, it.qty).total, soldBy: it.soldBy, unitLabel: it.unitLabel, step: it.step });
  });
  return items;
}
function buildTicket(data) {
  ...
  var lines = [];
  lines.push('*KaChu Domicilio*'); lines.push('');
  if (items && items.length) {
    ...
    items.forEach(function(it){
      ...
      if (pricing.breakdown && pricing.breakdown.length) {
        pricing.breakdown.forEach(function(b){ lines.push('> ' + b.times + ' × ' + b.qty + ' ' + unitTxt + ' = $' + Number(b.price).toFixed(2)); });
  ...
  if (split && split.splitEnabled){
    lines.push(''); lines.push('*Pago:* Dividido');
    ...
    lines.push(''); lines.push('*Pago:* ' + pay);
    if (efectivo) { lines.push('*Paga con:* $' + Number(efectivo.pagaCon).toFixed(2)); lines.push('*Cambio:* $' + Number(efectivo.cambio).toFixed(2)); }
  }
  lines.push('');
  if (address) { lines.push('*Dirección de entrega:*'); lines.push('> ' + address); lines.push(''); }
  lines.push('*Aviso:* _Hemos recibido tu solicitud, en un máximo de *15min-20min* te estaríamos entregando tu pedido_'); lines.push('');
...
function initSplitPay() {
  ...
  function bind(id, ev){
    ...
    if (el){ el.addEventListener(ev||'input', recalcSplitPay); }
  }
  sw.addEventListener('change', function(){
    box.classList.toggle('hidden', !sw.checked);
    recalcSplitPay(); validateCheckout();
  });
...
function collectSplitPayForOrder(){
  ...
  return {
    splitEnabled: true,
    payments: [ { method: m1, amount: +a1.toFixed(2) }, { method: m2, amount: +a2.toFixed(2) } ],
...
(function(){
  ...
  function initFab(fab){
    ...
    initAlternator(fab);
    function toggle(){ if (fab.classList.contains('open')) { fab.classList.remove('open'); btn.setAttribute('aria-expanded','false'); } else { closeAll(fab); fab.classList.add('open'); btn.setAttribute('aria-expanded','true'); } }
  ...
  if (document.readyState==='loading'){ window.addEventListener('DOMContentLoaded', initRail); } else { initRail(); }
})();
...
function favsApplyCenteredPadding(){
  ...
  favsTrack.style.paddingLeft = pad + 'px'; favsTrack.style.paddingRight = pad + 'px';
  favsTrack.dataset.slideW = String(slideW); favsTrack.dataset.gap = String(gap);
}
function favsSetupCenteredCarousel(){
  ...
  if (favsTrack && !favsTrack.dataset._groupBound) {
    favsTrack.addEventListener('scroll', function(){ favsBuildDotsByGroups(); updateFavsNavVisibility(); }, { passive: true });
    favsTrack.dataset._groupBound = '1';
  }
}
window.addEventListener('resize', function(){
  clearTimeout(window.__favsResizeT);
  window.__favsResizeT = setTimeout(function(){ favsApplyCenteredPadding(); favsBuildDotsByGroups(); updateFavsNavVisibility(); }, 100);
});
...
function renderFavoritesRail(){
  ...
  var items = Array.from(favorites).map(function(id){ return productsById.get(id); }).filter(Boolean);
  if (!items.length){
    favsTrack.innerHTML = ''; favsSection.classList.add('hidden'); favsBuildDotsByGroups(); updateFavsNavVisibility(); return;
  }
  ...
  favsTrack.innerHTML = items.map(function(p){
    ...
    var unitLabel = (p.soldBy === 'weight') ? 'kg' : 'pza';
    var isAvailable = (p.active !== false);
    var actions = isAvailable ? favQtyOrAddHTML(p) : '<button class="btn add unavailable" type="button" disabled aria-disabled="true" title="No disponible">No disponible</button>';
    return '' +
  ...
  favsTrack.onclick = function(e){
    ...
    if (e.target.closest('.btn.add') && !e.target.classList.contains('unavailable')){
      ...
      renderCart(); updateFavCardQtyUI(id); return;
    }
    var qplus = e.target.closest('.qplus'); var qminus = e.target.closest('.qminus');
    if (qplus || qminus){
      var it = cart.get(id);
      if (!it){
        var soldBy = p.soldBy || 'unit';
      ...
      renderCart(); updateFavCardQtyUI(id);
    }
  };
  favsSetupCenteredCarousel(); favsBuildDotsByGroups(); updateFavsNavVisibility(); refreshAllFavQtyUIs();
}
...
window.addEventListener('DOMContentLoaded', function(){
  (function fixTopbarOffset(){
    ...
    var applyOffset = function(){ var h = topbar.offsetHeight || 0; document.body.classList.add('has-fixed-topbar'); document.body.style.setProperty('--topbar-h', h + 'px'); };
    window.addEventListener('load', applyOffset);
    window.addEventListener('resize', applyOffset);
    if (document.readyState !== 'loading') applyOffset();
  })();

  loadServiceStatus().then(function(service){ if (!service.active) { showServiceOverlay(service); return; } }).catch(function(err){ console.error('Service status error', err); });

  loadCart(); initSplitPay(); recalcSplitPay(); renderCart(); validateCheckout(); loadCheckout();

  // Favoritos
  loadFavs(); favsSetupCenteredCarousel(); bindFavsRailEvents();
  ...
  Promise.all([ loadCategories(), loadProducts(), loadZones(), loadAds() ]).then(function(){
    ...
    toggleContinueButton(); updateFreeShippingPromo(); renderFavoritesRail(); updateAllCardHearts();
    if (zone.value && (cart.size > 0)) { fsShowOnAdd = true; updateFreeShippingPromo(); }
    bindAddButtons(); bindFavoriteHearts();
